...
type
  ...
  private type
    ...
    TArrayOfStr = TArray<string>;
    TArrayOfInt = TArray<integer>;

  var
    __vertexList: TListOfStr; // 存储顶点集合
    __edges: TArray<TArray<integer>>; // 存储图对应的邻结矩阵
    __numOfEdges: integer; // 表示边的数目
    __isVisitedForDFS: TArray<Boolean>; // 定义给数组, 记录 DFS 某个结点是否被访问
    __isVisitedForBFS: TArray<Boolean>; // 定义给数组, 记录 BFS 某个结点是否被访问
    // 插入结点
